...
4. 各参加者は最終的に `!ito submit` を使い自分が発表する数字を提出します（引数は不要）。
  - `!ito submit` は参加者に配布済みの「秘匿番号」をそのまま提出として記録します（引数に数字を含めないことで他の参加者に見えるリスクを防ぎます）。
...
6. 司会が `!ito end` を実行するとゲームは終了します。
  ...
  - ゲーム内で作成されたプライベートスレッドは終了時に削除されます（可能な場合）。
...
- `!ito submit`
  - 説明: 引数は不要。ボットはあなたに配布された秘匿番号をそのまま提出値として記録します。
  - 例: `!ito submit`
  - 返信例: "@ユーザー の提出を記録しました。"（DM に記録された値を送ることがあります。公開チャット上で値は表示されません。）
